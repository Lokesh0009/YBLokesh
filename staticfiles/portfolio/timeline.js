function Timeline(cvs) {
    ...
    self.options = {
        speed: 0.1,
        density: window.innerWidth < 768 ? 4 : 8,   // Lower density for mobile
        radius: window.innerWidth < 768 ? 300 : 600  // Smaller radius for mobile
    };
    ...
    function OnResize() {
        ...
        // Adjust line positions based on screen size
        var offsetX = window.innerWidth < 768 ? 10 : 15;  // Adjust offset for mobile
        self.lines[0].reset(canvas.offsetWidth / 2 - offsetX);
        self.lines[1].reset(canvas.offsetWidth / 2 + offsetX);
    ...
    function init() {
        ...
        function handlePointerMove(e) {
            if (e.touches) {
                ...
                mouse.x = e.touches[0].clientX - $(canvas).offset().left;
                mouse.y = e.touches[0].clientY - $(canvas).offset().top;
            } else {
                // Mouse event for desktop
                mouse.x = e.offsetX || e.layerX || e.pageX - $(canvas).offset().left;
                mouse.y = e.offsetY || e.layerY || e.pageY - $(canvas).offset().top;
            }
        }
    ...
    function redraw() {
        ...
        for (var i = 0; i < 2; i++) {
            ...
            // Draw the connecting lines and dots with a brighter color
            ctx.lineWidth = 0.5; // Increase thickness for connecting lines
            ctx.strokeStyle = self.lines[i].color; // Match color to the main line for brightness
            for (var j = 0; j < self.lines[i].dots.length; j++) {
                ...
                drawLine(p1, p2);  // Draw brighter connecting line
